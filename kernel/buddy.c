...
//add
//xor the bit of index(/2) position with 1
void bit_xor_2(char *array, int index){
  index = index/2;
  char b = array[index/8];
  char m = (1<<(index%8));
  array[index/8] = b^m;
...
//add
// Return 1 if bit at position index(/2) in array is set to 1
int bit_isset_2(char * array , int index){
  index = index / 2;
  char b = array[index/8];
  char m = (1 << (index % 8));
  return (b & m) == m;
...
bd_malloc(uint64 nbytes)
{
  ...
  bit_xor_2(bd_sizes[k].alloc, blk_index(k, p));
  for(; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k-1);   // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    bit_xor_2(bd_sizes[k-1].alloc, blk_index(k-1, p));
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_xor_2(bd_sizes[k].alloc, bi);  // free p at size k
    if (bit_isset_2(bd_sizes[k].alloc, buddy)) {  // is buddy allocated?
...
//pos ==0 means left boundry    ==1means right
int
bd_initfree_pair(int k, int bi,int pos) {
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  if(bit_isset_2(bd_sizes[k].alloc, bi))  {
